...
function onUpdate(elapsed)
    ...
    if (getMouseX('camHUD') > 5 and getMouseX('camHUD') < 132) and (getMouseY('camHUD') > 600 and getMouseY('camHUD') < 720 and mouseClicked('left')) or keyJustPressed('z') and DodgeDevilCreated == 0 or DodgeButtonPress and DodgeDevilCreated == 0 or getProperty('SmallDevil.x') > getProperty('boyfriend.x') - getProperty('boyfriend.width')/4 and getProperty('SmallDevil.x') < getProperty('boyfriend.x') + getProperty('boyfriend.width') /5 and botPlay and DodgeDevilCreated == 0 then
     DodgeDevilCreated = 1
